...
def test_security_headers(page):
    ...
    assert headers.get("x-content-type-options") == "nosniff", "X-Content-Type-Options header is not set to nosniff"
    assert headers.get("x-frame-options") == "SAMEORIGIN", "X-Frame-Options header is not set to SAMEORIGIN"
    assert headers.get("referrer-policy") == "same-origin", "Referrer-Policy header is not set to same-origin"

    # Check for HSTS if HTTPS_ONLY is enabled in your app.
    if "strict-transport-security" in headers:
        hsts = headers.get("strict-transport-security")
        assert hsts is not None, "Strict-Transport-Security header should be present when HTTPS_ONLY is enabled"
